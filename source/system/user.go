...
func (i *initUser) InitializeData(ctx context.Context) (next context.Context, err error) {
	...
	entities := []sysModel.SysUser{
		{
			...
			NickName:    "ZC LABS",
			HeaderImg:   "http://14.215.165.39:9535/888.png",
			AuthorityId: 888,
			Phone:       "18188888888",
		...
		},
		{
			...
			Password:    password,
			NickName:    "用户1",
			HeaderImg:   "http://14.215.165.39:9535/999.png",
			AuthorityId: 9528,
			Phone:       "19199999999",
			Email:       "333333333@qq.com"},
	}
	if err = db.Create(&entities).Error; err != nil {
		...
		return ctx, errors.Wrap(err, sysModel.SysUser{}.TableName()+"create data error!")
	}
	...
	if !ok {
		return next, errors.Wrap(system.ErrMissingDependentContext, "Unable to find permission table initialization data!")
	}
