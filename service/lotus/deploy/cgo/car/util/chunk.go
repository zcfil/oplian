...
func GenerateCarNew(fileList []Finfo, parentPath string, tmpDir string, output io.Writer) (ipldDag *FsNode, cid string, cidMap map[string]CidMapValue, err error) {
	...
	for _, item := range fileList {
		...
		node, err = BuildFileNode(context.TODO(), item, dagServ, cidBuilder)
		if err != nil {
			return
		}
		err = dagServ.Add(context.TODO(), node)
		if err != nil {
			return
		}
		path, err = filepath.Rel(filepath.Clean(parentPath), filepath.Clean(item.Name))
		if err != nil {
			return
		}
		...
		for j := len(previous) - 1; j >= i; j-- {
			...
			if ok {
				...
				err = dagServ.Add(context.TODO(), n)
				if err != nil {
					return
				}
				cidMap[strings.Join(previous[1:], "/")] = CidMapValue{true, n.Cid().String()}
				err = (*dirNode).AddChild(context.TODO(), lastName, n)
				if err != nil {
					return nil, "", nil, err
				}
			} else {
				lastFileNode, _ := lastNode.(ipld.Node)
				err = (*dirNode).AddChild(context.TODO(), lastName, lastFileNode)
				if err != nil {
					return nil, "", nil, err
				}
			}
	...
	err = dagServ.Add(context.TODO(), rootIpldNode)
	if err != nil {
		return
	}
	cidMap[""] = CidMapValue{true, rootIpldNode.Cid().String()}
	selector := allSelector()
	sc := car.NewSelectiveCar(context.TODO(), bs2, []car.Dag{{Root: rootIpldNode.Cid(), Selector: selector}})
	err = sc.Write(output)
	if err != nil {
		return
	}
